...
echo "Starting MySQL Docker container to apply seed file to database $DB_NAME on $DB_HOST"
docker run --rm \
  -v "$SCRIPT_DIR/seed_modified.sql:/seed_modified.sql" \
  mysql:8.0 \
  sh -c "mysql -h $DB_HOST -u $DB_USER -p'$DB_PASSWORD' < /seed_modified.sql"
